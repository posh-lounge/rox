...
import { Plus, X, Loader2, TrendingUp, Package, ChevronDown, ChevronLeft, ChevronRight } from "lucide-react";
...
function AddPurchaseModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState({ product_id: "", quantity: "", unit_cost: "", supplier_name: "", notes: "" });
  const set = (k: string, v: string) => setForm(f => ({ ...f, [k]: v }));
  const total = (+form.quantity || 0) * (+form.unit_cost || 0);

  const handleSubmit = async () => {
    if (!form.product_id || !form.quantity || !form.unit_cost) return;
    await addPurchase.mutateAsync({ product_id: +form.product_id, quantity: +form.quantity, unit_cost: +form.unit_cost, supplier_name: form.supplier_name, notes: form.notes } as any);
    onClose();
  };
      ...
      <div className="w-full max-w-md bg-gray-900 border border-white/10 rounded-2xl shadow-2xl">
        ...
        <div className="flex gap-3 px-6 pb-6">
          ...
          <button onClick={handleSubmit} disabled={addPurchase.isPending}
            ...
            {addPurchase.isPending && <Loader2 size={14} className="animate-spin" />} Add Purchase
          </button>
        </div>
      </div>
    </div>
