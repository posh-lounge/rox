...
function toggleDropdown(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
 ...
 <Skeleton height={24} width={100} borderRadius={8} />
        <span className="block mr-1 font-medium text-theme-sm">{sessiondata?.fullname}</span>
        </button>
        </div>
    )
  }
  if (status === "failed") {
    return <div>Error: {error}</div>; // Show error message if the request fails
  }
  if (status === "succeeded" && !sessiondata) {
    return <p>No session data available.</p>; // Handle case when no data is returned
  }
